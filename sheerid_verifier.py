...
from pdf_generator import render_pdf_from_template
...
# Force-disable DEBUG from noisy libraries
logging.getLogger("fontTools").setLevel(logging.WARNING)
logging.getLogger("weasyprint").setLevel(logging.WARNING)
logging.getLogger("PIL").setLevel(logging.WARNING)

# Optional but recommended: block propagation
...
class SheerIDVerifier:
    ...
    def create_verification(self) -> str:
        ...
        if (
            ...
            or not data.get("verificationId")
        ):
    ...
    def verify(
        ...
        try:
            ...
            DEFAULT_SUPPORT_EMAIL = "support@eduvision.live"
            ...
            # 生成学生 PNG
            logger.info("步骤 1/5: 生成学生 PNG 文档...")
            assets = render_pdf_from_template(first_name, last_name, write_local=True)

            logger.info(
                f"  - {assets['file_name']} 大小: {len(assets['data'])/1024:.2f}KB"
            )

            # 提交教师信息
            logger.info("步骤 2/5: 提交学生信息...")
            step2_body = {
            ...
            if (
                isinstance(step2_data, dict)
                and step2_data.get("currentStep") == "error"
            ):
                error_msg = ", ".join(step2_data.get("errorIds", ["Unknown error"]))
                raise Exception(f"步骤 2 错误: {error_msg}")

            logger.info(
                f"✅ 步骤 2 完成: {getattr(step2_data, 'get', lambda k, d=None: d)('currentStep')}"
            )
            current_step = (
            ...
            if current_step in ["sso", "collectTeacherPersonalInfo"]:
                ...
                logger.info(
                    f"✅ 步骤 3 完成: {getattr(step3_data, 'get', lambda k, d=None: d)('currentStep')}"
                )
                current_step = (
            ...
            if (
                step4_status != 200
                or not isinstance(step4_data, dict)
                or not step4_data.get("documents")
            ):
            ...
            if not self._upload_to_s3(upload_url, assets["data"]):
                raise Exception(f"S3 上传失败: {assets['file_name']}")

            logger.info(f"✅ 已上传 {assets['file_name']}")
            ...
            logger.info(
                f"✅ 文档提交完成: {getattr(step6_data, 'get', lambda k, d=None: d)('currentStep')}"
            )
...
def main():
    ...
    email_input = input(f"请输入联系邮箱 [{DEFAULT_SUPPORT_EMAIL}]: ").strip()

    email = email_input if email_input else DEFAULT_SUPPORT_EMAIL
